...
Перейдем к **поиску суммы на отрезке**. На вход поступает 2 числа ```l``` и ```r``` - индексы начала и конца отрезка. ```tl``` и ```tr``` - индексы начала и конца отрезка, соответствующего текущей вершине дерева. tl=0, tr=n-1

Пример реализации функции на Java: 

```java
// В основной программе вызовем sum(1, 0, n - 1, l, r).
int sum(int v, int tl, int tr, int l, int r) {
    // Если левая граница больше правой, отрезок пуст, возвращаем 0.
    if (l > r)
        return 0
            
    // Если текущий отрезок полностью содержится в заданном отрезке, возвращаем значение в текущем узле.
    if (l == tl && r == tr)
        return t[v];
    
    int tm = (tl + tr) / 2; // Середина текущего отрезка
    // Рекурсивно вызываем функцию для левого и правого подотрезков
    return sum(v * 2, tl, tm, l, Math.min(r, tm)) +
            sum(v * 2 + 1, tm + 1, tr, Math.max(l, tm + 1), r);
}
```

Высота дерева отрезков примерно равна log<sub>2</sub>n. Всякий раз у нас будет не более двух реально работающих ветвей рекурсии (можно сказать, что одна ветвь приближается к левой границе запроса, а вторая ветвь — к правой), а всего число затронутых отрезков не могло превысить высоты дерева отрезков, умноженной на 4, поэтому сложность поиска суммы на отрезке - ```O(log n)```.

Перейдем к **изменению значения в массиве**. Элемент ```a[i]``` участвует только в относительно небольшом числе вершин дерева отрезков: а именно, примерно в log<sub>2</sub>n вершинах - по одной с каждого уровня.

Тогда понятно, что запрос обновления можно реализовать как рекурсивную функцию: ей передаётся текущая вершина дерева отрезков, и эта функция выполняет рекурсивный вызов от одного из двух своих сыновей (от того, который содержит позицию i в своём отрезке), а после этого — пересчитывает значение суммы в текущей вершине точно таким же образом, как при построении дерева отрезков (сумма значений по обоим сыновьям текущей вершины). Получим сложность ```O(log n)```.
...
void update(int v, int tl, int tr, int pos, int new_val, int[] t) {
    if (tl == tr) {
        // Достигли листового узла, обновляем значение элемента.
        t[v] = new_val;
    } else {
        int tm = (tl + tr) / 2;
        ...
        // Обновляем значение в текущем узле.
        t[v] = t[v * 2] + t[v * 2 + 1];
    }
}
```
...
Такая задача очень похожа на задачу из билета 8.

**Постановка задачи:**  
Дан массив ```a```, содержащий ```n``` элементов. Необходимо обрабатывать запросы двух типов: запрос минимума (находить минимум среди элементов с ```l``` до ```r```) и запрос модификации (изменять значение указанного элемента массива).

Чтобы построить дерево отрезков для решения данной задачи. По заданному массиву дерево отрезков удобно строить снизу вверх: сначала запишем значения элементов ```a[i]``` в соответствующие листья дерева, затем на их основе посчитаем значения для вершин предыдущего уровня как **минимум** из значений в двух листьях, затем ещё одного, и так далее. В коде это можно реализовать в виде рекурсивной функции.

### 10. Алгоритм Форда-Беллмана. Описание алгоритма. Оценка времени выполнения.

**Алгоритм Форда-Беллмана** используется для нахождения кратчайшего пути между всеми парами вершин в ориентированном или неориентированном графе с возможностью существования отрицательных весов ребер.

Описание алгоритма Форда-Беллмана:

1. Инициализация: Устанавливаем начальную вершину, для которой мы ищем кратчайшие пути, в качестве текущей вершины. Для каждой вершины графа устанавливаем бесконечное расстояние от начальной вершины и 0 для начальной вершины.
2. Повторяем следующие шаги для каждой вершины в графе, итеративно обновляя расстояние до всех остальных вершин.
   - Для каждого ребра ```(u, v)``` в графе, где ```u``` - текущая вершина, ```v``` - смежная вершина:
     - Если расстояние до вершины ```v``` через текущую вершину ```u``` меньше, чем текущее расстояние до вершины ```v```, то обновляем расстояние до вершины ```v```.
   - Переходим к следующей итерации и обрабатываем все ребра графа.
3. Проверка наличия отрицательных циклов: После выполнения всех итераций алгоритма, выполняем дополнительную проверку наличия отрицательных циклов. Если на какой-то итерации произошло обновление расстояния до какой-либо вершины, это означает наличие отрицательного цикла в графе.
   - Для каждого ребра ```(u, v)``` в графе, где ```u``` - текущая вершина, ```v``` - смежная вершина:
     - Если расстояние до вершины ```v``` через текущую вершину u меньше, чем текущее расстояние до вершины ```v```, то граф содержит отрицательный цикл.

Второй шаг выполняется ```V - 1``` раз, где V``` - количество вершин. На каждой итерации происходит обновление расстояний до всех вершин графа через текущую вершину. Это означает, что каждое ребро графа будет рассмотрено ```V - 1``` раз. Получаем сложность ```O(V * E)```, где ```V``` - количество вершин в графе, а ```E``` - количество ребер.

### 11. Алгоритм Флойда-Уоршелла. Описание алгоритма. Оценка времени выполнения.

**Алгоритм Флойда-Уоршелла** используется для нахождения кратчайших путей между всеми парами вершин во взвешенном ориентированном графе. Он работает с графами, в которых рёбра могут иметь отрицательные веса.

Описание алгоритма Флойда-Уоршелла:

1. Создаем матрицу размером ```N * N```, где ```N``` - количество вершин в графе. Инициализируем ее значениями бесконечности для всех пар вершин, кроме пар ```(i, i)```, для которых значение равно 0.
2. Заполняем матрицу с помощью следующего шага:
   - Для каждого ребра ```(u, v)``` с весом ```w``` в графе:
     - Если w меньше значения в матрице для пары вершин ```(u, v)```, то обновляем значение в матрице на ```w```.
3. Для всех пар вершин ```(i, j)``` и промежуточных вершин ```(k)```:
   - Для каждой вершины ```k``` от 1 до N:
     - Для каждой вершины ```i``` от 1 до N:
       - Для каждой вершины ```j``` от 1 до N:
         - Если значение в матрице для пары вершин ```(i, j)``` больше суммы значений для пары вершин ```(i, k)``` и ```(k, j)```, то обновляем значение в матрице на сумму значений для пары вершин ```(i, k)``` и ```(k, j)```.

По окончании алгоритма матрица будет содержать кратчайшие расстояния между всеми парами вершин.

Оценка времени выполнения алгоритма Флойда-Уоршелла составляет ```O(N^3)```, где ```N``` - количество вершин в графе. Это связано с необходимостью выполнения трех вложенных циклов для обработки всех вершин и их пар.
